...
func executa() {
	...
	rowsTipo, err := db.Query("select agn_in_codigo, agn_tau_st_codigo, lead(agn_in_codigo, 1) over (order by agn_in_codigo) proximo_codigo from mgglo.glo_agentes_id where agn_pad_in_codigo = 1 and agn_in_codigo <> 2 order by agn_in_codigo")
